...
export async function onRequestPost(context) {
  ...
  if (!apiKey) {
    return json({ ok: false, error: '服务端缺少环境变量 QWEN_API_KEY' }, 500);
  }
  ...
  try {
    ...
    if (!image || typeof image !== 'string') {
      return json({ ok: false, error: '请提供 imageDataUrl' }, 400);
    }
    ...
    return json({ ok: true, result, raw: content });
  } catch (error) {
    return json({ ok: false, error: error?.message || '识别失败' }, 500);
  }
}
