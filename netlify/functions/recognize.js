...
export async function handler(event) {
  ...
  if (event.httpMethod !== 'POST') return json(405, { ok: false, error: '只支持 POST 请求' });

  const apiKey = process.env.QWEN_API_KEY || process.env.QREN_API_KEY;
  if (!apiKey) return json(500, { ok: false, error: '服务端缺少环境变量 QWEN_API_KEY' });

  try {
    const body = event.body ? JSON.parse(event.body) : {};
    const image = body.image || body.imageDataUrl || body.dataUrl;
    if (!image || typeof image !== 'string') return json(400, { ok: false, error: '请提供 imageDataUrl' });
    ...
    return json(200, { ok: true, result, raw: content });
  } catch (error) {
    return json(500, { ok: false, error: error?.message || '识别失败' });
  }
}
